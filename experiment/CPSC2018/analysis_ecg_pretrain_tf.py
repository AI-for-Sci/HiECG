...
class ProjectionHeader(tf.keras.models.Model):
    ...
    def call(self, hidden_states):
        ...
        if self.pooling == 'first-last-avg':
            ...
            output = tf.keras.layers.Lambda(lambda x: x[:, 0])(hidden_states)
        else:
            output = tf.keras.layers.Lambda(lambda x: x[:, 0])(hidden_states)
...
def main():
    ...
    if checkpoint is not None:
        model = TFSignal2VecModel.from_pretrained(checkpoint, config=config)
    # elif model_args.model_name_or_path:
    ...
    target_features = np.zeros((sample_size, output.shape[-1]))
    target_labels = np.zeros((sample_size,len(classes)))
    current_samples = 0
    ...
    for ii in range(len(classes)):
        ...
        num_samples = 10000
        sub_target_features = target_features[indexes[:num_samples]]
        sub_target_labels = target_labels[indexes[:num_samples]]

        sub_target_label_ids = sub_target_labels[:, ii]
        sub_target_label_names = []
        for label_id in sub_target_label_ids:
            if label_id == 1:
                sub_target_label_names.append(classes[ii])
            else:
                sub_target_label_names.append('other')

        anndata = sc.AnnData(sub_target_features)
        anndata.obs['Label'] = sub_target_label_names

        sc.pp.neighbors(anndata, n_neighbors=10, use_rep='X')  # default
        sc.tl.umap(anndata)
