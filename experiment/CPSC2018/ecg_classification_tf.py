...
class ClassificationHeader(tf.keras.models.Model):
    """
    """

    def __init__(self, pooling='cls', drop_rate=0.1, num_classes=10, hidden_size=128, hidden_norm=True, name='cls',
                 **kwargs):
        super(ClassificationHeader, self).__init__(name=name)
...
def train_step(training_args, model, project_header, optimizer, input_values, labels, train_accuracy):
...
# @tf.function
def distributed_train_step(training_args, model, project_header, optimizer, input_values, labels, train_accuracy):
    """
    """
...
def main():
    ...
    #     config = Signal2VecConfig.from_pretrained(model_args.model_name_or_path)
    else:
    ...
    if data_args.train_file is not None:
        ...
        train_dataset = train_dataset.padded_batch(batch_size=training_args.per_device_train_batch_size,
                                                   ...
                                                   padding_values=(0.0, 0.0), drop_remainder=True)
    ...
    if data_args.validation_file is not None:
        val_dataset = load_dataset(data_args.validation_file)
        val_dataset = val_dataset.padded_batch(batch_size=training_args.per_device_eval_batch_size,
                                               padded_shapes=([None, None], [None]),
                                               padding_values=(0.0, 0.0), drop_remainder=True)
        val_dataset = val_dataset.prefetch(tf.data.experimental.AUTOTUNE)

        # strategy.scope创建并分发数据集
        val_dataset = training_args.strategy.experimental_distribute_dataset(val_dataset)
    ...
    with training_args.strategy.scope():
        ...
        if model_args.model_name_or_path:
            model = TFSignal2VecModel.from_pretrained(model_args.model_name_or_path, config=config)
        else:
        ...
        for epoch in range(int(training_args.num_train_epochs)):
            ...
            for step, (input_values, labels) in enumerate(train_dataset):
                ...
                if step % 100 == 0 and step > 0:
                    template = 'Epoch {}, Step {}, Loss: {:.6f}, Accuracy: {:.6f}.'
            ...
            true_labels = np.zeros((sample_size, len(classes)), dtype=np.float)
            pred_labels = np.zeros((sample_size, len(classes)), dtype=np.float)
            loss_fn = tf.keras.losses.BinaryCrossentropy(
                from_logits=True, reduction=tf.keras.losses.Reduction.NONE
            )
            current_samples = 0
            for step, (input_values, labels) in enumerate(val_dataset):
                if current_samples + len(labels) > sample_size:
                    break

                z1 = model({"input_values": input_values}, output_hidden_states=True, training=False)
                logits = project_header(z1['hidden_states'], training=False)

                true_labels[current_samples:current_samples + len(labels)] = labels
                pred_labels[current_samples:current_samples + len(labels)] = logits

                loss = loss_fn(labels, logits)
                val_loss(loss)
                val_accuracy(labels, logits)

                current_samples += len(labels)

            true_labels = true_labels[:current_samples, :]
            pred_labels = pred_labels[:current_samples, :]
            ...
            f1s = cal_f1s(true_labels, pred_labels)
            avg_f1 = np.mean(f1s)
            print('F1s:', f1s)
            print('Avg F1: %.4f' % avg_f1)

            np.set_printoptions(precision=3)
            template = 'Epoch {}, Loss: {:.6f}, Accuracy: {:.6f}, Val_Loss: {:.6f}, Val_Accuracy: {:.6f}, Times: {}'
            print(template.format(epoch + 1,
                                  train_loss.result(),
                                  train_accuracy.result(),
                                  val_loss.result(),
            ...
            if training_args.output_dir is not None:
                ...
                project_header.save_weights(os.path.join(training_args.output_dir, 'classification_header.h5'),
                                            save_format='h5')
