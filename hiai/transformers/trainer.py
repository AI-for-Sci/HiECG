...
class Trainer:
    ...
    Args:
        ...
        train_dataset (:obj:`torch.utils.preprocess.Dataset` or :obj:`torch.utils.preprocess.IterableDataset`, `optional`):
            The dataset to use for training. If it is an :obj:`datasets.Dataset`, columns not accepted by the
            ``model.forward()`` method are automatically removed.

            Note that if it's a :obj:`torch.utils.preprocess.IterableDataset` with some randomization and you are training in
        ...
        eval_dataset (:obj:`torch.utils.preprocess.Dataset`, `optional`):
             The dataset to use for evaluation. If it is an :obj:`datasets.Dataset`, columns not accepted by the
             ``model.forward()`` method are automatically removed.
        tokenizer (:class:`PreTrainedTokenizerBase`, `optional`):
            The tokenizer used to preprocess the preprocess. If provided, will be used to automatically pad the inputs the
    ...
    Important attributes:
        ...
        - **is_model_parallel** -- Whether or not a model has been switched to a model parallel mode (different from
          preprocess parallelism, this means some of the model layers are split on different GPUs).
    ...
    def _remove_unused_columns(self, dataset: "datasets.Dataset", description: Optional[str] = None):
        ...
        if self._signature_columns is None:
            ...
            # Labels may be named label or label_ids, the default preprocess collator handles that.
    ...
    def get_train_dataloader(self) -> DataLoader:
        """
        Returns the training :class:`~torch.utils.preprocess.DataLoader`.
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        """
        Returns the evaluation :class:`~torch.utils.preprocess.DataLoader`.
    ...
    def get_test_dataloader(self, test_dataset: Dataset) -> DataLoader:
        """
        Returns the test :class:`~torch.utils.preprocess.DataLoader`.

        Subclass and override this method if you want to inject some custom behavior.

        Args:
            test_dataset (:obj:`torch.utils.preprocess.Dataset`, `optional`):
    ...
    def num_examples(self, dataloader: DataLoader) -> int:
        """
        Helper to get number of samples in a :class:`~torch.utils.preprocess.DataLoader` by accessing its dataset.
        ...
        if resume_from_checkpoint is not None and os.path.isfile(
            ...
            if not args.ignore_data_skip:
                logger.info(
                    ...
                    "flag to your launch command, but you will resume the training on preprocess already seen by your model."
                )
    ...
    def _prepare_input(self, data: Union[torch.Tensor, Any]) -> Union[torch.Tensor, Any]:
        """
        Prepares one :obj:`preprocess` before feeding it to the model, be it a tensor or a nested list/dictionary of tensors.
        """
