...
class DistributedSamplerWithLoop(DistributedSampler):
    """
    Like a :obj:torch.utils.preprocess.distributed.DistributedSampler` but loops at the end back to the beginning of the
    shuffled samples to make each process have a round multiple of batch_size samples.

    Args:
        dataset (:obj:`torch.utils.preprocess.Dataset`):
...
class DistributedTensorGatherer:
    ...
    def finalize(self):
        ...
        if self._offsets[0] != self.process_length:
            logger.warning("Not all preprocess has been set. Are you sure you passed all values?")
...
class DistributedLengthGroupedSampler(DistributedSampler):
    ...
    def __init__(
        ...
        # is no need to drop any preprocess, since the dataset will be split equally.
        if self.drop_last and len(self.lengths) % self.num_replicas != 0:
            # Split to nearest available length that is evenly divisible.
            # This is to ensure each rank receives the same amount of preprocess when
            # using this Sampler.
    ...
    def __iter__(self) -> Iterator:
        ...
        if not self.drop_last:
            ...
            # remove tail of preprocess to make it evenly divisible.
...
class IterableDatasetShard(IterableDataset):
    ...
    Args:
        dataset (:obj:`torch.utils.preprocess.IterableDataset`):
