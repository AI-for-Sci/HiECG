...
class FlaxSignal2VecBaseModelOutput(ModelOutput):
    """
    Output type of :class:`~transformers.FlaxSignal2VecBaseModelOutput`, with potential hidden states and attentions.
...
@flax.struct.dataclass
class FlaxSignal2VecForPreTrainingOutput(ModelOutput):
    """
    Output type of :class:`~transformers.FlaxSignal2VecForPreTrainingOutput`, with potential hidden states and
    attentions.
...
WAV_2_VEC_2_INPUTS_DOCSTRING = r"""
    Args:
        ...
        attention_mask (:obj:`jnp.ndarray` of shape :obj:`(batch_size, sequence_length)`, `optional`):
            ...
            whose processor has ``config.return_attention_mask == False``, such as `Signal2Vec-base
            <https://huggingface.co/facebook/Signal2Vec-base-960h>`__, :obj:`attention_mask` should **not** be passed to
...
class FlaxSignal2VecLayerNormConvLayer(nn.Module):
    config: Signal2VecConfig
    layer_id: int = 0
...
class FlaxSignal2VecPositionalConvEmbedding(nn.Module):
    config: Signal2VecConfig
    dtype: jnp.dtype = jnp.float32
...
class FlaxConvLayersCollection(nn.Module):
    ...
    def setup(self):
        if self.config.feat_extract_norm == "layer":
            self.layers = [
                FlaxSignal2VecLayerNormConvLayer(self.config, layer_id=i, name=str(i), dtype=self.dtype)
...
class FlaxSignal2VecFeatureExtractor(nn.Module):
...
class FlaxSignal2VecFeatureProjection(nn.Module):
    config: Signal2VecConfig
    dtype: jnp.dtype = jnp.float32
...
class FlaxSignal2VecAttention(nn.Module):
    config: Signal2VecConfig
    embed_dim: int
    num_heads: int
    dropout: float = 0.0
...
class FlaxSignal2VecFeedForward(nn.Module):
    config: Signal2VecConfig
    dtype: jnp.dtype = jnp.float32
...
class FlaxSignal2VecEncoderLayerStableLayerNorm(nn.Module):
    config: Signal2VecConfig
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.attention = FlaxSignal2VecAttention(
            config=self.config,
        ...
        self.layer_norm = nn.LayerNorm(epsilon=self.config.layer_norm_eps, dtype=self.dtype)
        self.feed_forward = FlaxSignal2VecFeedForward(self.config, dtype=self.dtype)
...
class FlaxSignal2VecEncoderLayerStableLayerNormCollection(nn.Module):
    config: Signal2VecConfig
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.layers = [
            FlaxSignal2VecEncoderLayerStableLayerNorm(self.config, name=str(i), dtype=self.dtype)
...
class FlaxSignal2VecStableLayerNormEncoder(nn.Module):
    config: Signal2VecConfig
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.pos_conv_embed = FlaxSignal2VecPositionalConvEmbedding(self.config, dtype=self.dtype)
        self.layer_norm = nn.LayerNorm(epsilon=self.config.layer_norm_eps, dtype=self.dtype)
        self.dropout = nn.Dropout(rate=self.config.hidden_dropout)
        self.layers = FlaxSignal2VecEncoderLayerStableLayerNormCollection(self.config, dtype=self.dtype)
...
class FlaxSignal2VecGumbelVectorQuantizer(nn.Module):
    """
...
class FlaxSignal2VecPreTrainedModel(FlaxPreTrainedModel):
    """
    ...
    base_model_prefix: str = "Signal2Vec"
...
class FlaxSignal2VecModule(nn.Module):
    config: Signal2VecConfig
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.feature_extractor = FlaxSignal2VecFeatureExtractor(self.config, dtype=self.dtype)
        self.feature_projection = FlaxSignal2VecFeatureProjection(self.config, dtype=self.dtype)
        self.masked_spec_embed = self.param(
            "masked_spec_embed", jax.nn.initializers.uniform(), (self.config.hidden_size,)
        )

        if self.config.do_stable_layer_norm:
            self.encoder = FlaxSignal2VecStableLayerNormEncoder(self.config, dtype=self.dtype)
        else:
    ...
    def __call__(
        ...
        return FlaxSignal2VecBaseModelOutput(
...
class FlaxSignal2VecModel(FlaxSignal2VecPreTrainedModel):
    module_class = FlaxSignal2VecModule


FLAX_Signal2Vec_MODEL_DOCSTRING = """
    Returns:

    Example::

        >>> from transformers import Signal2VecProcessor, FlaxSignal2VecModel
        >>> from datasets import load_dataset
        >>> import soundfile as sf

        >>> processor = Signal2VecProcessor.from_pretrained("facebook/Signal2Vec-large-lv60")
        >>> model = FlaxSignal2VecModel.from_pretrained("facebook/Signal2Vec-large-lv60")
...
overwrite_call_docstring(
    FlaxSignal2VecModel,
    WAV_2_VEC_2_INPUTS_DOCSTRING + FLAX_Signal2Vec_MODEL_DOCSTRING,
)
append_replace_return_docstrings(
    FlaxSignal2VecModel, output_type=FlaxSignal2VecBaseModelOutput, config_class=Signal2VecConfig
)


class FlaxSignal2VecForCTCModule(nn.Module):
    config: Signal2VecConfig
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.Signal2Vec = FlaxSignal2VecModule(self.config, dtype=self.dtype)
...
class FlaxSignal2VecForCTC(FlaxSignal2VecPreTrainedModel):
    module_class = FlaxSignal2VecForCTCModule


FLAX_Signal2Vec_FOR_CTC_DOCSTRING = """
    Returns:

    Example::

        >>> import jax.numpy as jnp
        >>> from transformers import Signal2VecProcessor, FlaxSignal2VecForCTC
        >>> from datasets import load_dataset
        >>> import soundfile as sf

        >>> processor = Signal2VecProcessor.from_pretrained("facebook/Signal2Vec-large-960h-lv60")
        >>> model = FlaxSignal2VecForCTC.from_pretrained("facebook/Signal2Vec-large-960h-lv60")
...
overwrite_call_docstring(
    ...
    WAV_2_VEC_2_INPUTS_DOCSTRING + FLAX_Signal2Vec_FOR_CTC_DOCSTRING,
)
append_replace_return_docstrings(FlaxSignal2VecForCTC, output_type=FlaxCausalLMOutput, config_class=Signal2VecConfig)
...
class FlaxSignal2VecForPreTrainingModule(nn.Module):
    config: Signal2VecConfig
    dtype: jnp.dtype = jnp.float32
    ...
    def setup(self):
        ...
        self.quantizer = FlaxSignal2VecGumbelVectorQuantizer(self.config, dtype=self.dtype)
    ...
    def __call__(
        ...
        return FlaxSignal2VecForPreTrainingOutput(
...
class FlaxSignal2VecForPreTraining(FlaxSignal2VecPreTrainedModel):
    module_class = FlaxSignal2VecForPreTrainingModule
...
FLAX_Signal2Vec_FOR_PRETRAINING_DOCSTRING = """
    Returns:
    ...
    Example::
        ...
        >>> import numpy as np
        >>> import jax.numpy as jnp
        >>> from transformers import Signal2VecFeatureExtractor, FlaxSignal2VecForPreTraining
        >>> from transformers.models.Signal2Vec.modeling_flax_Signal2Vec import _compute_mask_indices
        >>> from datasets import load_dataset
        >>> import soundfile as sf

        >>> feature_extractor = Signal2VecFeatureExtractor.from_pretrained("facebook/Signal2Vec-large-lv60")
        >>> model = FlaxSignal2VecForPreTraining.from_pretrained("facebook/Signal2Vec-large-lv60")
...
overwrite_call_docstring(
    ...
    WAV_2_VEC_2_INPUTS_DOCSTRING + FLAX_Signal2Vec_FOR_PRETRAINING_DOCSTRING,
)
append_replace_return_docstrings(
    FlaxSignal2VecForPreTraining, output_type=FlaxSignal2VecForPreTrainingOutput, config_class=Signal2VecConfig
)
