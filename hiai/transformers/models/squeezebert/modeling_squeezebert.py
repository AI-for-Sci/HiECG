...
class SqueezeBertLayerNorm(nn.LayerNorm):
    """
    This is a nn.LayerNorm subclass that accepts NCW preprocess layout and performs normalization in the C dimension.
...
class SqueezeBertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_attentions):
        """
        expects hidden_states in [N, C, W] preprocess layout.

        The attention_mask preprocess layout is [N, W], and it does not need to be transposed.
        """
...
SQUEEZEBERT_START_DOCSTRING = r"""
    ...
    Data layouts::
        ...
        Input preprocess is in [batch, sequence_length, hidden_size] format.

        Data inside the encoder is in [batch, hidden_size, sequence_length] format. But, if :obj:`output_hidden_states
        == True`, the preprocess from inside the encoder is returned in [batch, sequence_length, hidden_size] format.
